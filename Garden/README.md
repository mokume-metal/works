...
**`v0.7.0` → `v0.9.0` で、全画素が動いた。** 4 枚 (frame 1 / 61 / 121 / 181) とも 160000 画素のほぼ全部で、差の平均は 5.5〜6.5 / 255。**青の面がいちばん大きい** (12) のに対し、赤と緑は 2〜4 にとどまる。

**正体は色の作業空間で、背景 1 画素で言い切れる。** このスケッチの背景は原典の `background("lightblue")` を数で写した `background(173, 216, 230)` である。

| | PNG の生のバイト | sRGB へ直すと |
| --- | --- | --- |
| `v0.7.0` | `(173, 216, 230)` — 打った数そのもの | `(161, 218, 232)` |
| `v0.9.0` | `(182, 215, 228)` | `(173, 216, 230)` — **打った数そのもの** |

**打った数が出る先が、バイト列から色そのものへ移った。** 書き出す PNG はどちらの版も Display P3 を刻むので、旧版は「P3 の原色で 173, 216, 230」= sRGB より彩度の高い水色を出していた。新版は数を sRGB の原色として受けるので、**原典の `lightblue` と同じ色**が出る。

改善の方向なので mokume へは戻していない (works 自身が [mokume#911](https://github.com/mokume-metal/mokume/issues/911) として戻したものが塞がった結果である)。**上の絵は貼り替えていない** — 並べても区別が付かず、Grain が「10 画素は目で見て分からない」として据え置いたのと同じ判断による。
