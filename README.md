...
| [Cast](Cast/) | 1 つの塊が、向きによって円・三角・四角の影を落とす。**影を落とす 1 本目**で、読めるのは影だけ — 塊は距離の関数 `f ≤ 0` として彫ってあり、**影が狙いの外へ出ないことが定義から出る** (欠けだけが起きる)。箱を積まずに 0 面から面を切り出すのも、被覆を立ち上げに数えて名乗るのもここが初めて |

**8 作品が mokume `v0.7.0` を引いている** ([#21](https://github.com/mokume-metal/works/issues/21))。**Pond・Quarry・Tempo・Cast が `v0.7.1`** で、作った日にそこから解決されたのがその版だった — 残りの追随は [#49](https://github.com/mokume-metal/works/issues/49) が追っている。
