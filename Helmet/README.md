...
**`v0.7.0` → `v0.9.0` で、兜の面だけが動いた。** 960x540 のうち 10.0〜11.6% (4 枚) で、これは**兜が占める面積とほぼ同じ**である (観測が返す `contentFraction` は 0.11)。背景の暗い平面はほとんど動いていない。

**窓を開けて並べて見たが、見た目は変わらない。** 稜線に白い線は載っていない — `v0.9.0` は読み込んだモデルにも `stroke()` を引くようになったが、この作品は `noStroke()` の中で置いているので当たらない。

**動いたのは彩度のある画素だけ**で、これは色の作業空間の変更どおりの効き方である (切り分けは [Garden](../Garden/README.md#v090-で動いたもの) が背景 1 画素で示している)。

**mokume が黙るようになった。** `v0.7.0` では起動のたびに 2 本の警告が出ていた:

```
mokume: スタイルの積み降ろしはフレームの中でだけできます。初期化のときに積んだスタイルはどのフレームにも属さないため、無視しました
mokume: 変換はフレームごとに書き直すものなので、描くところ (draw) で呼んでください。初期化のときに書いた変換はどのフレームにも属さないため、無視しました
```

`v0.9.0` では 1 本も出ない (観測の目録も `warnings: []`)。**無視されていた呼び出しが無くなったのか、無視しても言わなくなったのかは、この版上げでは切り分けていない。**

**切り分けの限界も書いておく。** 時計は実時間 (`Clock.wallClock`) なので、**同じ版で 2 回撮っても一致しない** — 実測した床は 8.95〜10.87% / 0.76〜0.89 で、版上げの差 (10.0〜11.6% / 1.23〜1.44) はそれをわずかに上回るだけである。上の「同じフレーム番号からは同じ絵が出る」が成り立つのは時計を `frameIndex` にしたときで、窓を開けて走らせる既定では成り立たない。

**上の絵は貼り替えていない** — 並べても区別が付かない。
