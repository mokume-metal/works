...
struct Track {
    ...
    /// 制御点 (メートル)。z が北。**先頭がスタート / フィニッシュ線**で、そこが `s = 0` になる。
    ///
    /// 主ストレート → 高速右 → 中速右 → バックストレート → **ヘアピン** → S 字 →
    /// 最終右。**速いコーナーと遅いコーナーを両方持たせる**のは、同じ 1 つの式から出る
    /// 限界速度の差 (54 km/h と 120 km/h) を走って確かめられるようにするため。
    ///
    /// ## グリッドを本当の直線に置く
    ///
    /// スタートラインの手前 6〜37 m にグリッドを並べる。以前は線を原点に置いていたので、
    /// **グリッドが最終右の出口に乗っていた** — その範囲で向きが 30.7° 変わり、グリッドの
    /// 向きのまま踏むと 3 秒で草に落ちた (#88)。
    ///
    /// いまは原点から北へ **x = 0 の上に 5 点を並べ**、線をその途中の (0, 65) へ進めてある。
    /// 向心 Catmull-Rom は、一直線に並んだ 4 点の中の 2 点の間を厳密に直線で結ぶので、
    /// グリッドの範囲の向きの変化は 0 になる。最終右の形はほとんど変わらない
    /// (最小半径 16.9 m → 16.2 m)
    private static let control: [SIMD2<Float>] = [
        SIMD2(0, 65),  // スタート / フィニッシュ
        SIMD2(0, 115),
        SIMD2(0, 158),  // 主ストレートの終わり
        SIMD2(22, 222),
        SIMD2(84, 262),  // T1 高速右
        SIMD2(158, 268),
        SIMD2(220, 244),
