...
![虹の輪 (頂点数 6)](https://github.com/user-attachments/assets/ea0dde98-8bdf-42c0-a045-afc95effa12a)
...
![外から送ったマウス位置で頂点数が 6 から 60 へ変わる](https://github.com/user-attachments/assets/a1e42063-350b-4aa9-8ce5-8ae63238ee23)
...
**`v0.7.0` → `v0.9.0` で、帯だけが動いた。** 288000 画素のうち 30551 (10.6%) で、これは**虹の帯そのものの面積**である。4 枚 (frame 1 / 61 / 121 / 181) の差がすべて同じなのは、この絵が時刻に依らないためである。

**背景の黒は 1 ビットも動いていない** — 動いた 30551 画素のうち、前後どちらかが背景と同じ色だったのは 89 画素しかない。灰色 (赤・緑・青が等しい色) は新しい作業空間でも動かないという約束どおりである。

| | |
| --- | --- |
| 動いた画素のうちの平均の差 | 9.5 / 255 |
| 最大の差 | 255 |
| 絵に出てくる色の種類 | 11217 → **17462** |

**色数が 1.6 倍に増えたのが、いちばん効いた変化である。** 頂点ごとの色は `map` で色相を回して作っているので、旧版では原色の付近で頭打ちになって**同じ値へ潰れていた**。数を sRGB の原色として受けるようになった結果、その潰れが解けて階調が戻った。目で見ると、原色へ振り切っていた緑とマゼンタの彩度が落ち着いて見える。

**原典に近づいた方向なので mokume へは戻していない** — 数と色の対応は works 自身が [mokume#911](https://github.com/mokume-metal/mokume/issues/911) として戻したもので、そこが塞がった結果である。

**この作品は毎回まったく同じ絵を出す**ので、上の数はすべて版差である (同じ `v0.9.0` で 2 回撮って 1 ビットも違わないことを確かめた)。

**上の 2 枚は `v0.9.0` で撮り直したものへ貼り替えた** ([#76](https://github.com/mokume-metal/works/issues/76))。この 13 本のなかで、彩度の落ち着きが目で見て分かるのはこの作品だけである。

**上げ先は Gyazo ではなく GitHub の添付になっている。** 貼り替えようとした時点で Upload API は 502 を返し続けており、しかも**それまでに上げた 67 枚が全部 404 になっていた** — 上げ直す先が無いので、手元のファイルをブラウザから Issue のコメント欄へ貼って `github.com/user-attachments/…` を得ている (手順は mokume の `visual-evidence` スキル)。他の 12 本の README はまだ絵を失ったままで、[#80](https://github.com/mokume-metal/works/issues/80) が追っている。

**貼り替えた絵で色数を数え直すと 17480 で、上の表の 17462 より 18 多い。** 頂点数はマウスの位置からしか決まらないので 6 から 60 まで 55 通り振って数えたが、**17462 になる位置は 1 つも無かった** (頂点数 6 が最大で 17480、7 で 14747、以降は減る)。絵の側は 3 回撮ってもバイト一致し、`run` と `watch` でも一致するので、**食い違っているのは #75 の測り方のほう**である。表は測ったときの記録としてそのまま残す。
