...
final class Prism: Sketch {
    ...
    private func automaticSource() -> SIMD2<Float> {
        // 163 度 ± 7 度。**入射角でいえば 40〜54 度を往復する** — 浅い側の 6 度が
        // 全反射の窓で、そこへ入ると紫から順に色が剥がれる。
        //
        // **深い側を 54 度で止めているのは、角を避けるためである。** 60 度では束の
        // 中心が左下の頂点をちょうど通り、どちらの面に当たったのかが決まらない
        // (中心の光線 1 本で追っているので、そこでは絵が壊れる)
        let sweep = radians(163) + radians(7) * sin(2 * Float.pi * time / 16)
